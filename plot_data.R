...
# plot function
plot_value <- function(data, type = c("daily", "cumulative"), title, hr = FALSE) {
  match.arg(type, choices = c("daily", "cumulative"), several.ok = FALSE)
  if (type == "daily") {y <- "value_daily"} else {y <- "value"}
  if (hr) {
    ggplot(data = data, aes(x = date, y = !!sym(y), colour = province,
...
## cases
plot_value(cases_prov, type = "daily", title = "Daily case data")
ggsave("plots/cases_prov.png", width = 731, height = 603, units = "px", dpi = 96)
plot_value(cases_hr, type = "daily", title = "Daily case data (health regions)", hr = TRUE)
ggsave("plots/cases_hr.png", width = 731, height = 603, units = "px", dpi = 96)

## mortality
plot_value(mortality_prov, type = "daily", title = "Daily mortality data")
ggsave("plots/mortality_prov.png", width = 731, height = 603, units = "px", dpi = 96)
plot_value(mortality_hr, type = "daily", title = "Daily mortality data (health regions)", hr = TRUE)
